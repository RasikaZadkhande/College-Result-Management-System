...
class Register:
    def __init__(self, root):
        ...
        # ======= background Image ==========
        self.bg = ImageTk.PhotoImage(file="loginsystem/images/bg.png")
        bg = Label(self.root, image=self.bg)
        bg.place(x=300, y=0, relwidth=1, relheight=1)

        # ======= left Image ==========
        self.left = ImageTk.PhotoImage(file="loginsystem/images/side.png")
        ...
        self.btn_img=ImageTk.PhotoImage(file="loginsystem/images/register.png")
        btn_register=Button(frame1,image=self.btn_img,bd=0,cursor="hand2",command=self.register_data).place(x=50,y=420) 

        btn_login=Button(left,text="Sign In",font=("times new roman",20),command=self.login_window,bg="#0b5799", fg="White", cursor="hand2", activebackground="#084b83", activeforeground="white").place(x=150,y=440)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()==""or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from employee2 where email=?",(self.txt_email.get(),))
                row=cur.fetchone()
                #print(row)
                if row!=None:
                    messagebox.showerror("Error", "User already Exists,Please try with another email", parent=self.root)
                else:
                    cur.execute("insert into employee2(f_name,l_name,contact,email,question,answer,password) values(?,?,?,?,?,?,?)",
                            (self.txt_fname.get(),
                             self.txt_lname.get(),
